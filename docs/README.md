# Рекомендательная система прогнозирования и предупреждения возникновения аварийных ситуаций на водоканале
# Основной: https://карта-водоканала.рф
# Зеркало: https://watermap.ofmaps.ru
# главная фишка: сценарно-ориентированный подход!

## Оглавление
- [Интерфейс предиктивно-рекомендательной системы на базе сценарного подхода](#интерфейс-предиктивно-рекомендательной-системы-на-базе-сценарного-подхода)
  - [Описание](#описание)
  - [Установка](#установка)
  - [Использование](#использование)
  - [Требования](#требования)
- [Оценка качества прогнозирования различных подходов](#оценка-качества-прогнозирования-различных-подходов)
  - [Как выглядит результат моделирования](#как-выглядит-результат-моделирования)
- [Сценарии, причины и последствия аварийных ситуаций](#сценарии-причины-и-последствия-аварийных-ситуаций)
  - [Исследование и классификация аварий на городских водоканалах мира](#исследование-и-классификация-аварий-на-городских-водоканалах-мира)
    - [Что чаще всего ломается в городской воде](#что-чаще-всего-ломается-в-городской-воде)
    - [Классификация сценариев](#классификация-сценариев)
    - [Исследование исторических сценариев с эффектом домино](#исследование-исторических-сценариев-которые-развиваются-с-цепной-реакцией-эффект-домино)
      - [Почему важен сценарный подход на примере зимнего шторма Uri (Техас, февраль 2021)](#почему-важен-сценарный-подход-на-историческом-примере-сценарий-зимнего-шторма-uri-техас-февраль-2021)
  - [Типовые сценарии для моделирования](#типовые-сценарии-для-моделирования)
    - [1) Прорыв магистрального водовода](#1-прорыв-магистрального-водовода)
    - [2) Отказ насосной станции (МНС/НС водоснабжения)](#2-отказ-насосной-станции-мнсны-водоснабжения)
    - [3) Отключение электропитания + не стартует ДГУ](#3-отключение-электропитания--не-стартует-дгу)
    - [4) Срыв коагуляции на ВПУ (рост мутности)](#4-срыв-коагуляции-на-впу-рост-мутности)
    - [5) Срыв дезинфекции (остаточный хлор ≈ 0 или > нормы)](#5-срыв-дезинфекции-остаточный-хлор--0-или--нормы)
    - [6) Загрязнение источника (нефтепродукты/токсины)](#6-загрязнение-источника-нефтепродуктытоксины)
    - [7) Массовый приток в канализацию (ливень/таяние) — риск переливов](#7-массовый-приток-в-канализацию-ливеньтаяние--риск-переливов)
    - [8) Засор/коллапс канализационного коллектора](#8-засорколлапс-канализационного-коллектора)
    - [9) Утечка хлора (газ) на хлораторной](#9-утечка-хлора-газ-на-хлораторной)
    - [10) Резкий рост недоучтённой воды (NRW) в DMA](#10-резкий-рост-недоучтённой-воды-nrw-в-dma)
    - [11) Киберинцидент/сбой SCADA](#11-киберинцидентсбой-scada)
    - [12) Некачественное топливо/неисправность ДГУ](#12-некачественное-топливонеисправность-дгу)
    - [13) Сбой промывки фильтров (не срабатывает автоматика)](#13-сбой-промывки-фильтров-не-срабатывает-автоматика)
    - [14) Контаминация резервуара (биоплёнка/водоросли)](#14-контаминация-резервуара-биоплёнкаводоросли)
  - [Упрощенная схема сценарного подхода](#упрощенная-схема-для-объяснения-как-работает-сценарный-подход-на-практике)
- [Пример работы рекомендательного сервиса в интерфейсе системы](#пример-работы-рекомендательного-сервиса-в-интерфейсе-системы)
...
* **LGBM** демонстрирует наилучшее качество прогноза (MAE≈0.03, SMAPE≈20%), заметно опережая **Linear Regression** (SMAPE≈80%) и **Transformer** (SMAPE≈33%). При этом LGBM еще и самый быстрый и выгодный в плане аппаратных ресурсов.
...
### Исследование и классификация аварий на городских водоканалах мира
Подробно: https://docs.google.com/spreadsheets/d/1E6D1bqdESWux7cXtjK5i6diYW7pzIFU_8fJ5mgt-tPY/edit?gid=0#gid=0
#### Что чаще всего ломается» в городской воде:
* Питьевая вода — порывы/утечки труб (массовые и повсеместные; главная «рутинная» авария), затем перерывы из-за электропитания/насосного оборудования. 
* Канализация — переливы/сбросы (SSO) и аварии на сооружениях при ливнях. 
* Хлор/реагенты — менее частые в абсолюте, но высокорисковые случаи, требующие строгой ПБ и газоаналитики. 
* Инциденты качества воды/заболеваемости — чаще связаны с Legionella и уязвимостями распределительных/внутридомовых систем.
...
#### Исследование исторических сценариев, которые развиваются с цепной реакцией (эффект домино)
Подробно: https://docs.google.com/spreadsheets/d/1E6D1bqdESWux7cXtjK5i6diYW7pzIFU_8fJ5mgt-tPY/edit?gid=542516407#gid=542516407

Критическая цепь, приводящая к эффекту домино: триггер (погода/энергия/человеческий фактор) → насос/щит/датчик → падение давления / переливы / загрязнение → межсекторные эффекты (больницы, транспорт, экология).

![domino](https://github.com/Flaysty/watermap/blob/main/docs/scenario/water_domino_scenarios.png)
Граф типовых цепных реакций

##### Почему важен сценарный подход на историческом примере: сценарий зимнего шторма Uri (Техас, февраль 2021) 
| Когда | Что произошло |
|---|---|
| 15–16 фев 2021 | Сильный мороз. Генерация упала в связи с обледенением. |
| 16–17 фев | Из-за веерных отключений электричества и холода остановились/замёрзли насосы и станции очистки воды. |
| 17 фев (день–вечер) | Давление в сети стало слишком низким. |
| 17–19 фев | Трубы в домах и под землёй лопались от заморозков. |
| 18–21 фев | Мороз ослаб, подача энергии стабилизировалось, но сеть ещё приходилось ремонтировать/поднимать давление. |
| 21–23 фев | Приборы показали норму, давление восстановили. |
...
### Упрощенная схема для объяснения, как работает сценарный подход на практике:
![Concept](https://github.com/Flaysty/watermap/blob/main/docs/scenario/concept.png)
1. Из систем мониторинга поступают данные, которые преобразуются в показатели (есть скрипт для генерации синтетических для тестов)
2. Технолог может настроить пороги для отлавливания отклонений 
3. Есть типовые сценарии, которые характеризуются набором аномалий
4. Модель отслеживает изменения параметров, прогнозирует их, что мэтчится с типовыми сценариями
5. После этого на базе типовых сценариев выбираются рекомендации к действию по предотвращению
